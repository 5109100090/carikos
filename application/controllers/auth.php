...
class Auth extends CI_Controller
{
	...
	function __construct(){
		...
		$this->load->library(array('Form_validation', 'session', 'DX_Auth'));
	...
	function email_check($email)
	{
		...
		if ( ! $result)
		{
			$this->form_validation->set_message('email_check', 'Email sudah digunakan, silahkan gunakan email yang lain.');
		}
	...
	private function show_message($message, $redirect){
		//$message = $this->dx_auth->get_auth_error();
		//$this->session->set_flashdata('message', $message);
		echo $message.'<br />redirecting..';
		$base_url = base_url().$redirect;
		echo '<script type="text/javascript">alert(m);window.location.href = base;</script>';
	}
	...
	function login()
	{
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			else
			{
				...
				{						
					...
					//$this->session->set_flashdata('message', $message);
					//$base_url = base_url();
					//echo "<script>var m = '".$message."'; var base = '".$base_url."'; alert(m);window.location = base;</script>";
					//redirect('/');
					//$this->show_message($message, null);
				}
			}
		}
		else
		{
			...
			//$this->show_message('You are already logged in.', 'panel');
	...
	{		
		...
		{	
			...
			if ($val->run() AND $this->dx_auth->register(md5($val->set_value('email')), $val->set_value('password'), $val->set_value('email')))
			{	
				// Set success message accordingly
				if ($this->dx_auth->email_activation)
				{
					//$data['auth_message'] = 'You have successfully registered. Check your email address to activate your account.';
					//$this->show_message('You have successfully registered. Check your email address to activate your account.', null);
					echo 'You have successfully registered. Check your email address to activate your account.';
				}
				else
				{					
					//$data['auth_message'] = 'You have successfully registered. '.anchor(site_url($this->dx_auth->login_uri), 'Login');
					//$this->show_message("You have successfully registered.", null);
		...
		elseif ( ! $this->dx_auth->allow_registration)
		{
			...
			//$this->show_message('Registration has been disabled.', null);
		...
		else
		{
			...
			//$this->load->view($this->dx_auth->logged_in_view, $data);
			//$this->show_message('You have to logout first, before registering.', 'panel');
			echo 'You have to logout first, before registering.';
		}
	}
	...
	function activate()
	{
		...
		if ($this->dx_auth->activate($username, $key)) 
		{
			...
			$data['auth_message'] = 'Your account have been successfully activated. '.anchor(site_url(), 'Login');
